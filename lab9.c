...
void insertRecord(struct HashType *hashTable, struct RecordType *record, int tableSize)
{
	...
	else
	{
		...
		struct RecordType* current = hashTable->table[index];
...
void initHashTable(int tableSize, struct HashType** hashTable)
{
	*hashTable = (struct HashType*)calloc(1,sizeof(struct HashType));
	
	(*hashTable)->tableSize = tableSize;
	
	(*hashTable)->table = (struct RecordType**)calloc(tableSize,sizeof(struct RecordType*));
